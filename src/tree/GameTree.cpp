...
// gpt generated
json GameTree::jsonify_tree(const Node *node,
                            const PreflopRangeManager &prm) const {
  json ret;
  ...
  case NodeType::ACTION_NODE: {
    ...
    // pull out averaged strategy
    const auto strat = action_node->get_average_strat();
    const auto &hands = prm.get_preflop_combos(action_node->get_player());
    ...
    for (size_t h = 0; h < num_hands; ++h) {
      json hand_json;
      hand_json["hand"] = hands[h].to_string();
      ...
      for (size_t a = 0; a < num_actions; ++a) {
        ...
        ajson["type"] = type_str;
        ajson["amount"] = actions[a].amount;
        ajson["prob"] = strat[h + a * num_hands];
        actions_arr.push_back(std::move(ajson));
      }

      hand_json["actions"] = std::move(actions_arr);
      strategy.push_back(std::move(hand_json));
    }
    ret["strategy"] = std::move(strategy);

    // recurse into children
    ret["children"] = json::array();
    for (size_t i = 0; i < num_actions; ++i) {
      const Node *child = action_node->get_child(i);
      json child_json = jsonify_tree(child, prm);

      // tag with the action taken to reach this child
      std::string type_str;
      ...
      child_json["action"] = type_str;
      child_json["amount"] = actions[i].amount;
      ret["children"].push_back(std::move(child_json));
    }
  } break;
  ...
  case NodeType::CHANCE_NODE: {
    ...
    ret["type"] = "chance_node";
    ret["chance_type"] =
        (chance_node->get_type() == ChanceNode::ChanceType::DEAL_TURN)
            ? "DEAL_TURN"
            : "DEAL_RIVER";

    ret["children"] = json::array();
    for (int i = 0; i < chance_node->get_num_children(); ++i) {
      const Node *c = chance_node->get_child(i);
      if (!c)
        continue;
      ret["children"].push_back(jsonify_tree(c, prm));
    }
  } break;

  case NodeType::TERMINAL_NODE: {
    auto term = dynamic_cast<const TerminalNode *>(node);
    ret["type"] = "terminal_node";
    ...
    case TerminalNode::TerminalType::ALLIN:
      ...
      break;
    case TerminalNode::TerminalType::UNCONTESTED:
      term_str = "UNCONTESTED";
      break;
    case TerminalNode::TerminalType::SHOWDOWN:
      term_str = "SHOWDOWN";
      break;
    }
    ret["terminal_type"] = term_str;
    ret["last_to_act"] = term->get_last_to_act();
    ret["pot"] = term->get_pot();
  } break;
  }
