...
public:
  ...
  constexpr static uint64_t max() { return ~(uint64_t)0; }
...
public:
  ...
  template <class TRng> T operator()(TRng &rng) {
...
template <typename T = unsigned> class FastUniformIntDistribution2 {
public:
  typedef T TUnsigned;
...
private:
  template <class TRng> void getBits(TRng &rng) {
    do {
      mBuffer = rng() & mMask;
